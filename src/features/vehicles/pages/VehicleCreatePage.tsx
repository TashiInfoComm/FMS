...
type FieldDef = {
  key: keyof VehicleFormStringState
  label: string
  placeholder: string
...
export function VehicleFormPage() {
  ...
  const validate = (): string | null => {
    ...
    if (!form.vehicle_category_id.trim()) return 'Vehicle category is required.'
    if (!form.fuel_type_id.trim()) return 'Fuel type is required.'
    return null
  }
                  ...
                  {group.fields.map((field) =>
                    field.key === 'asset_name_id' && !isEdit ? (
                      ...
                      <MasterDataSelect
                        ...
                        onValueChange={(next) => setForm((prev) => ({ ...prev, [field.key]: next }))}
                      />
                    ) : (
                      <div key={field.key} className="space-y-2">
                        <Label htmlFor={field.key}>

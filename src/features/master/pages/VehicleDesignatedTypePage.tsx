// Manages vehicle designated usage type master data from `/master/usage-types` CRUD endpoints.
...
function mapRows(records: ApiRecord[], serialStart: number): UsageTypeRow[] {
...
function buildStatusPayload(row: UsageTypeRow, active: boolean) {
  return {
    code: row.code.trim(),
    name: row.name.trim(),
...
function listPath(search: string, page: number, pageSize: number) {
  ...
  return `/master/usage-types?page=${page}&page_size=${pageSize}&code=&search=${q}`
}

/** Paginated CRUD for vehicle designated usage types (`/master/usage-types`). */
export function VehicleDesignatedTypePage() {
  ...
  const crud = useRouteCrudPermissions('/master/designated-type')

  const listQuery = useQuery({
    queryKey: ['master-usage-types', search, page, pageSize],
    queryFn: async () => {
  ...
  const createMutation = useMutation({
    mutationFn: (payload: Record<string, string | number | boolean>) =>
      apiPost<unknown, Record<string, string | number | boolean>>('/master/usage-types', payload),
    onSuccess: () => {
      showSuccessToast('Vehicle designated type created successfully')
      queryClient.invalidateQueries({ queryKey: ['master-usage-types'] })
    },
    onError: (error) => {
      const message =
        error instanceof Error ? error.message : 'Failed to create vehicle designated type'
  ...
  const updateMutation = useMutation({
    mutationFn: ({
      ...
      apiPut<unknown, Record<string, string | number | boolean>>(
        `/master/usage-types/${encodeURIComponent(code)}`,
        payload,
      ),
    onSuccess: () => {
      showSuccessToast('Vehicle designated type updated successfully')
      queryClient.invalidateQueries({ queryKey: ['master-usage-types'] })
    },
    onError: (error) => {
      const message =
        error instanceof Error ? error.message : 'Failed to update vehicle designated type'
      showErrorToast(message)
    },
  })
  ...
  const deleteMutation = useMutation({
    mutationFn: (code: string) =>
      apiDelete<unknown>(`/master/usage-types/${encodeURIComponent(code)}`),
    onSuccess: () => {
      showSuccessToast('Vehicle designated type deactivated successfully')
      queryClient.invalidateQueries({ queryKey: ['master-usage-types'] })
    },
    onError: (error) => {
      const message =
        error instanceof Error ? error.message : 'Failed to deactivate vehicle designated type'
      showErrorToast(message)
    },
  })
  ...
  const onEdit = (row: UsageTypeRow) => {
    if (!crud.canUpdate) return
    if (!row.code) {
      showErrorToast('Missing code for vehicle designated type update')
      return
    }
  ...
  const onDeleteRequest = (row: UsageTypeRow) => {
    if (!crud.canDelete) return
    if (!row.code) {
      showErrorToast('Missing code for vehicle designated type delete')
      return
    }
  ...
  const onSubmit = (raw: FormValues) => {
    ...
    const existingRow = editingCode ? rows.find((row) => row.code === editingCode) : undefined
    const payload = {
      code: raw.code.trim(),
      name: raw.name.trim(),
  ...
  const onToggleStatus = (row: UsageTypeRow, checked: boolean) => {
    if (!crud.canUpdate) return
    if (!row.code) {
      showErrorToast('Missing code for vehicle designated type status update')
      return
    }
  ...
  return (
    <section className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <PageHeader
          title="Vehicle Designated Type"
          subtitle="Manage vehicle designated usage type records and configurations"
        />
        <Dialog
          open={dialogOpen}
          onOpenChange={(open) => {
        ...
        >
          ...
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>
                {editingCode ? 'Update Vehicle Designated Type' : 'Add New Vehicle Designated Type'}
              </DialogTitle>
            </DialogHeader>
          ...
          <div className="flex justify-end">
            <div className="relative w-full max-w-sm">
              ...
              <Input
                ...
                placeholder="Search vehicle designated type..."
                className="pl-9"
              />
            </div>
          </div>
      ...
      <DeleteDialog
        ...
        title="Deactivate Vehicle Designated Type"
        description="Are you sure you want to deactivate this vehicle designated type? This action cannot be undone."
      />
    </section>
  )
}

export default VehicleDesignatedTypePage

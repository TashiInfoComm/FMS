// Manages fuel type master data from API-backed CRUD endpoints.
...
function mapRows(records: ApiRecord[], serialStart: number): FuelTypeRow[] {
...
function buildStatusPayload(row: FuelTypeRow, active: boolean) {
  return {
    code: row.code.trim(),
    name: row.name.trim(),
...
function listPath(search: string, page: number, pageSize: number) {
  ...
  return `/master/fuel-types?page=${page}&page_size=${pageSize}&code=&search=${q}`;
}

/** Same UX pattern as DestinationPage but bound to `/master/fuel-types` endpoints and row shape. */
export function FuelTypePage() {
  ...
  const crud = useRouteCrudPermissions("/master/fuel-type");

  const listQuery = useQuery({
    queryKey: ["master-fuel-types", search, page, pageSize],
    queryFn: async () => {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      showSuccessToast("Fuel type created successfully");
      queryClient.invalidateQueries({ queryKey: ["master-fuel-types"] });
    },
    onError: (error) => {
      const message =
  ...
  const updateMutation = useMutation({
    mutationFn: ({
      ...
      apiPut<unknown, Record<string, string | number | boolean>>(
        `/master/fuel-types/${encodeURIComponent(code)}`,
        payload,
      ),
    onSuccess: () => {
      showSuccessToast("Fuel type updated successfully");
      queryClient.invalidateQueries({ queryKey: ["master-fuel-types"] });
    },
    onError: (error) => {
      const message =
  ...
  const deleteMutation = useMutation({
    mutationFn: (code: string) =>
      apiDelete<unknown>(`/master/fuel-types/${encodeURIComponent(code)}`),
    onSuccess: () => {
      showSuccessToast("Fuel type deleted successfully");
      queryClient.invalidateQueries({ queryKey: ["master-fuel-types"] });
    },
    onError: (error) => {
      const message =
        error instanceof Error ? error.message : "Failed to delete fuel type";
      showErrorToast(message);
    },
  });
  ...
  const onEdit = (row: FuelTypeRow) => {
    if (!crud.canUpdate) return;
    if (!row.code) {
      showErrorToast("Missing code for fuel type update");
      return;
    }
  ...
  const onDeleteRequest = (row: FuelTypeRow) => {
    if (!crud.canDelete) return;
    if (!row.code) {
      showErrorToast("Missing code for fuel type delete");
      return;
    }
  ...
  const onToggleStatus = (row: FuelTypeRow, checked: boolean) => {
    if (!crud.canUpdate) return;
    if (!row.code) {
      showErrorToast("Missing code for fuel type status update");
      return;
    }
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        >
          ...
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>
                {editingCode ? "Update Fuel Type" : "Add New Fuel Type"}
              </DialogTitle>
            </DialogHeader>
            ...
            >
              ...
              <div className="space-y-2">
                <Label htmlFor="name">
                  Fuel Type <span className="text-[var(--fms-delete)]">*</span>
                </Label>
                <Input
                  id="name"
                  {...register("name", {
                    ...
                    validate: (value) =>
                      value.trim() !== "" || "Fuel type cannot be empty",
                  })}
          ...
          <div className="hidden overflow-x-auto rounded-lg border border-[var(--fms-strokes)] md:block">
            <table className="min-w-full text-sm">
              <thead className="bg-[#f6f6f7] text-[var(--fms-text-header)]">
                <tr>
                  {["Sl.No", "Code", "Fuel Type", "Description", "Status"].map(
                    (column) => (
                      <th
                        key={column}

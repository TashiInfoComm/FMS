// Manages insurance provider master data from API-backed CRUD endpoints.
...
function mapRows(records: ApiRecord[], serialStart: number): InsuranceProviderRow[] {
...
function buildStatusPayload(row: InsuranceProviderRow, active: boolean) {
  return {
    code: row.code.trim(),
    name: row.name.trim(),
...
function listPath(search: string, page: number, pageSize: number) {
  ...
  return `/master/insurance-providers?page=${page}&page_size=${pageSize}&code=&search=${q}`
}

/** Paginated CRUD for insurance providers (same dialog/search pattern as DestinationPage). */
export function InsuranceProviderPage() {
  ...
  const crud = useRouteCrudPermissions('/master/insurance-provider')

  const listQuery = useQuery({
    queryKey: ['master-insurance-providers', search, page, pageSize],
    queryFn: async () => {
  ...
  const createMutation = useMutation({
    mutationFn: (payload: Record<string, string | number | boolean>) =>
      apiPost<unknown, Record<string, string | number | boolean>>('/master/insurance-providers', payload),
    onSuccess: () => {
      showSuccessToast('Insurance provider created successfully')
      queryClient.invalidateQueries({ queryKey: ['master-insurance-providers'] })
    },
    onError: (error) => {
      const message = error instanceof Error ? error.message : 'Failed to create insurance provider'
  ...
  const updateMutation = useMutation({
    mutationFn: ({ code, payload }: { code: string; payload: Record<string, string | number | boolean> }) =>
      apiPut<unknown, Record<string, string | number | boolean>>(
        `/master/insurance-providers/${encodeURIComponent(code)}`,
        payload,
      ),
    onSuccess: () => {
      showSuccessToast('Insurance provider updated successfully')
      queryClient.invalidateQueries({ queryKey: ['master-insurance-providers'] })
    },
    onError: (error) => {
      const message = error instanceof Error ? error.message : 'Failed to update insurance provider'
      showErrorToast(message)
    },
  })
  ...
  const deleteMutation = useMutation({
    mutationFn: (code: string) =>
      apiDelete<unknown>(`/master/insurance-providers/${encodeURIComponent(code)}`),
    onSuccess: () => {
      showSuccessToast('Insurance provider deleted successfully')
      queryClient.invalidateQueries({ queryKey: ['master-insurance-providers'] })
    },
    onError: (error) => {
      const message = error instanceof Error ? error.message : 'Failed to delete insurance provider'
      showErrorToast(message)
    },
  })
  ...
  const onEdit = (row: InsuranceProviderRow) => {
    if (!crud.canUpdate) return
    if (!row.code) {
      showErrorToast('Missing code for insurance provider update')
      return
    }
  ...
  const onDeleteRequest = (row: InsuranceProviderRow) => {
    if (!crud.canDelete) return
    if (!row.code) {
      showErrorToast('Missing code for insurance provider delete')
      return
    }
  ...
  const onToggleStatus = (row: InsuranceProviderRow, checked: boolean) => {
    if (!crud.canUpdate) return
    if (!row.code) {
      showErrorToast('Missing code for insurance provider status update')
      return
    }
  ...
  return (
    <section className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <PageHeader title="Insurance Provider" subtitle="Manage insurance provider records and configurations" />
        <Dialog
          open={dialogOpen}
          onOpenChange={(open) => {
            setDialogOpen(open)
        ...
        >
          ...
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>{editingCode ? 'Update Insurance Provider' : 'Add New Insurance Provider'}</DialogTitle>
            </DialogHeader>
            ...
            <form className="space-y-3 py-1" onSubmit={handleSubmit(onSubmit)} noValidate>
              ...
              <div className="space-y-2">
                <Label htmlFor="name">
                  Insurance Provider <span className="text-[var(--fms-delete)]">*</span>
                </Label>
                <Input
                  id="name"
                  {...register('name', {
                    required: 'Insurance provider is required',
                    validate: (value) => value.trim() !== '' || 'Insurance provider cannot be empty',
                  })}
                  placeholder="Enter insurance provider"
                  aria-invalid={formState.errors.name ? true : undefined}
                />
                {formState.errors.name?.message ? <p className="text-xs text-[var(--fms-delete)]">{formState.errors.name.message}</p> : null}
              </div>
              ...
              <div className="space-y-2">
                ...
                />
                {formState.errors.description?.message ? <p className="text-xs text-[var(--fms-delete)]">{formState.errors.description.message}</p> : null}
              </div>
          ...
          <div className="hidden overflow-x-auto rounded-lg border border-[var(--fms-strokes)] md:block">
            <table className="min-w-full text-sm">
              <thead className="bg-[#f6f6f7] text-[var(--fms-text-header)]">
                <tr>
                  {['Sl.No', 'Code', 'Insurance Provider', 'Description', 'Status'].map((column) => (
              ...
              <tbody>
                {listQuery.isLoading ? (
                  ...
                  rows.map((row, index) => (
                    <tr key={`insurance-provider-${index}`} className="border-t border-[var(--fms-strokes)]">
                      ...
                      <td className="px-4 py-3">
                        <div className="inline-flex items-center gap-2">
                          <Switch checked={row.active} onCheckedChange={(checked) => onToggleStatus(row, checked)} disabled={!crud.canUpdate || updateMutation.isPending} />
                          <span className="text-xs text-[var(--fms-text-subheading)]">{row.active ? 'Active' : 'Inactive'}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3">

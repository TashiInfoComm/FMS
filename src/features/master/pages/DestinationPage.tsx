// Manages destination master data from API-backed CRUD endpoints.
...
function mapRows(records: ApiRecord[], serialStart: number): DestinationRow[] {
...
function buildStatusPayload(row: DestinationRow, active: boolean) {
  return {
    code: row.code.trim(),
    name: row.name.trim(),
...
function listPath(search: string, page: number, pageSize: number) {
  ...
  return `/master/destinations?page=${page}&page_size=${pageSize}&code=&search=${q}`
}

/**
 * Standard master CRUD screen: paginated GET, create/update/delete mutations, dialog forms,
 * and `code` as the business key for edit/delete selection (`editingCode`, `selectedDeleteCode`).
 */
export function DestinationPage() {
  ...
  const crud = useRouteCrudPermissions('/master/destination')

  const listQuery = useQuery({
    queryKey: ['master-destinations', search, page, pageSize],
    queryFn: async () => {
  ...
  const createMutation = useMutation({
    mutationFn: (payload: Record<string, string | number | boolean>) =>
      apiPost<unknown, Record<string, string | number | boolean>>('/master/destinations', payload),
    onSuccess: () => {
      showSuccessToast('Destination created successfully')
      queryClient.invalidateQueries({ queryKey: ['master-destinations'] })
    },
    onError: (error) => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ code, payload }: { code: string; payload: Record<string, string | number | boolean> }) =>
      apiPut<unknown, Record<string, string | number | boolean>>(
        `/master/destinations/${encodeURIComponent(code)}`,
        payload,
      ),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (code: string) =>
      apiDelete<unknown>(`/master/destinations/${encodeURIComponent(code)}`),
    onSuccess: () => {
  ...
  const onEdit = (row: DestinationRow) => {
    if (!crud.canUpdate) return
    if (!row.code) {
      showErrorToast('Missing code for destination update')
      return
    }
  ...
  const onDeleteRequest = (row: DestinationRow) => {
    if (!crud.canDelete) return
    if (!row.code) {
      showErrorToast('Missing code for destination delete')
      return
    }
  ...
  const onToggleStatus = (row: DestinationRow, checked: boolean) => {
    if (!crud.canUpdate) return
    if (!row.code) {
      showErrorToast('Missing code for destination status update')
      return
    }
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        >
          ...
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>
                {editingCode ? "Update Destination" : "Add New Destination"}
              </DialogTitle>
            </DialogHeader>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  {...register("name", {
                    ...
                    validate: (value) =>
                      value.trim() !== "" || "Destination cannot be empty",
                  })}

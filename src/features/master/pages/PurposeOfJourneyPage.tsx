// Manages purpose of journey master data from API-backed CRUD endpoints.
...
function mapRows(records: ApiRecord[], serialStart: number): JourneyPurposeRow[] {
...
function buildStatusPayload(row: JourneyPurposeRow, active: boolean) {
  return {
    code: row.code.trim(),
    name: row.name.trim(),
...
function listPath(search: string, page: number, pageSize: number) {
  ...
  return `/master/journey-purposes?page=${page}&page_size=${pageSize}&code=&search=${q}`
}

/** Paginated CRUD for journey purpose codes (`/master/journey-purposes`). */
export function PurposeOfJourneyPage() {
  ...
  const crud = useRouteCrudPermissions('/master/purpose-of-journey')

  const listQuery = useQuery({
    queryKey: ['master-journey-purposes', search, page, pageSize],
    queryFn: async () => {
  ...
  const createMutation = useMutation({
    mutationFn: (payload: Record<string, string | number | boolean>) =>
      apiPost<unknown, Record<string, string | number | boolean>>('/master/journey-purposes', payload),
    onSuccess: () => {
      showSuccessToast('Purpose of journey created successfully')
      queryClient.invalidateQueries({ queryKey: ['master-journey-purposes'] })
    },
    onError: (error) => {
      const message = error instanceof Error ? error.message : 'Failed to create purpose of journey'
  ...
  const updateMutation = useMutation({
    mutationFn: ({ code, payload }: { code: string; payload: Record<string, string | number | boolean> }) =>
      apiPut<unknown, Record<string, string | number | boolean>>(
        `/master/journey-purposes/${encodeURIComponent(code)}`,
        payload,
      ),
    onSuccess: () => {
      showSuccessToast('Purpose of journey updated successfully')
      queryClient.invalidateQueries({ queryKey: ['master-journey-purposes'] })
    },
    onError: (error) => {
      const message = error instanceof Error ? error.message : 'Failed to update purpose of journey'
      showErrorToast(message)
    },
  })
  ...
  const deleteMutation = useMutation({
    mutationFn: (code: string) =>
      apiDelete<unknown>(`/master/journey-purposes/${encodeURIComponent(code)}`),
    onSuccess: () => {
      showSuccessToast('Purpose of journey deleted successfully')
      queryClient.invalidateQueries({ queryKey: ['master-journey-purposes'] })
    },
    onError: (error) => {
      const message = error instanceof Error ? error.message : 'Failed to delete purpose of journey'
      showErrorToast(message)
    },
  })
  ...
  const onEdit = (row: JourneyPurposeRow) => {
    if (!crud.canUpdate) return
    if (!row.code) {
      showErrorToast('Missing code for purpose of journey update')
      return
    }
  ...
  const onDeleteRequest = (row: JourneyPurposeRow) => {
    if (!crud.canDelete) return
    if (!row.code) {
      showErrorToast('Missing code for purpose of journey delete')
      return
    }
  ...
  const onToggleStatus = (row: JourneyPurposeRow, checked: boolean) => {
    if (!crud.canUpdate) return
    if (!row.code) {
      showErrorToast('Missing code for purpose of journey status update')
      return
    }
  ...
  return (
    <section className="space-y-5">
        ...
        >
          ...
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>
                {editingCode
                  ? "Update Purpose Of Journey"
                  : "Add New Purpose Of Journey"}
              </DialogTitle>
            </DialogHeader>
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <Input
                  id="name"
                  {...register("name", {
                    required: "Purpose of journey is required",
                    validate: (value) =>
                      value.trim() !== "" ||
                      "Purpose of journey cannot be empty",
                  })}
          ...
          <div className="flex justify-end">
            <div className="relative w-full max-w-sm">
              ...
              <Input
                ...
                placeholder="Search purpose of journey..."
                className="pl-9"
              />
            </div>
          </div>
      ...
      <DeleteDialog
        ...
        title="Delete Purpose Of Journey"
        description="Are you sure you want to delete this purpose of journey? This action cannot be undone."
      />
    </section>
  );
}

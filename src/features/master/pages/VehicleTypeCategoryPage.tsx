...
function listPath(basePath: string, search: string, page: number, pageSize: number) {
...
function MasterSection({ title, subtitle, basePath, queryKey, deleteTitle }: MasterSectionProps) {
  ...
  const listQuery = useQuery({
    ...
    queryFn: async () => {
      const payload = await apiGet<unknown>(listPath(basePath, search, page, pageSize))
  ...
  const createMutation = useMutation({
    ...
    onError: (error) => {
      const message = error instanceof Error ? error.message : `Failed to create ${title.toLowerCase()}`
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      const message = error instanceof Error ? error.message : `Failed to update ${title.toLowerCase()}`
      showErrorToast(message)
    },
  })
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error) => {
      const message = error instanceof Error ? error.message : `Failed to delete ${title.toLowerCase()}`
      showErrorToast(message)
    },
  })
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        >
          ...
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>{editingCode ? `Update ${title}` : `Add New ${title}`}</DialogTitle>
            </DialogHeader>
            ...
            <form className="space-y-3 py-1" onSubmit={handleSubmit(onSubmit)} noValidate>
              ...
              <div className="space-y-2">
                <Label htmlFor="name">
                  {title} <span className="text-[var(--fms-delete)]">*</span>
                </Label>
                <Input
                  id="name"
                  {...register('name', {
          ...
          <div className="hidden overflow-x-auto rounded-lg border border-[var(--fms-strokes)] md:block">
            <table className="min-w-full text-sm">
              <thead className="bg-[#f6f6f7] text-[var(--fms-text-header)]">
                <tr>
                  {['Sl.No', 'Code', title, 'Description', 'Status', 'Actions'].map((column) => (
              ...
              <tbody>
                {listQuery.isLoading ? (
                  ...
                  rows.map((row, index) => (
                    <tr key={`${queryKey}-${index}`} className="border-t border-[var(--fms-strokes)]">

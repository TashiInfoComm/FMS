...
function buildStatusPayload(row: MaintenanceTypeRow, active: boolean) {
  return {
    code: row.code.trim(),
    name: row.name.trim(),
...
function listPath(search: string, page: number, pageSize: number) {
  ...
  return `/master/maintenance-types?page=${page}&page_size=${pageSize}&code=&search=${q}`
}
...
export function MaintenanceTypePage() {
  ...
  const listQuery = useQuery({
    queryKey: ['master-maintenance-types', search, page, pageSize],
    queryFn: async () => {
  ...
  const createMutation = useMutation({
    mutationFn: (payload: Record<string, string | number | boolean>) =>
      apiPost<unknown, Record<string, string | number | boolean>>('/master/maintenance-types', payload),
    onSuccess: () => {
      showSuccessToast('Maintenance type created successfully')
      queryClient.invalidateQueries({ queryKey: ['master-maintenance-types'] })
    },
    onError: (error) => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ code, payload }: { code: string; payload: Record<string, string | number | boolean> }) =>
      apiPut<unknown, Record<string, string | number | boolean>>(
        `/master/maintenance-types/${encodeURIComponent(code)}`,
        payload,
      ),
    onSuccess: () => {
      showSuccessToast('Maintenance type updated successfully')
      queryClient.invalidateQueries({ queryKey: ['master-maintenance-types'] })
    },
    onError: (error) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (code: string) =>
      apiDelete<unknown>(`/master/maintenance-types/${encodeURIComponent(code)}`),
    onSuccess: () => {
      showSuccessToast('Maintenance type deleted successfully')
      queryClient.invalidateQueries({ queryKey: ['master-maintenance-types'] })
    },
    onError: (error) => {
  ...
  const onEdit = (row: MaintenanceTypeRow) => {
    if (!crud.canUpdate) return
    if (!row.code) {
      showErrorToast('Missing code for maintenance type update')
      return
    }
  ...
  const onDeleteRequest = (row: MaintenanceTypeRow) => {
    if (!crud.canDelete) return
    if (!row.code) {
      showErrorToast('Missing code for maintenance type delete')
      return
    }
  ...
  const onToggleStatus = (row: MaintenanceTypeRow, checked: boolean) => {
    if (!crud.canUpdate) return
    if (!row.code) {
      showErrorToast('Missing code for maintenance type status update')
      return
    }
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  {...register("name", {
                    ...
                    validate: (value) =>
                      value.trim() !== "" || "Maintenance type cannot be empty",
                  })}
